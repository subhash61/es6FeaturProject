...
class Parks extends Elements {
    ...
    calculateAge() {
        var age = new Date().getFullYear() - this.buidlyr;
        return age;
    }
...
class Streets extends Elements {
        ...
        streettypes () {
            ...
            return streetanalysis.get (this.size);
        }
...
function parkReport (p) {
    ...
    p.forEach(el => {
        console.log(`${el.name} has a tree density of ${el.ParkDensity()} trees per square km`);
        sum += el.calculateAge();   
    })
    avg = sum/p.length;
    console.log(`Our ${p.length} parks have an average of ${avg} years.`);
    p.map (el => {
        if(el.noOfTrees >= 1000) {
...
function streetReport (s) {
    ...
    s.forEach(el => {
        console.log(`${el.name} build in ${el.buidlyr}, is a ${el.streettypes()} `)
        streetSum += el.totalLength;
    })
    var avgLength = streetSum/s.length;
    console.log(`Our ${s.length} streets have a total length of ${streetSum} km , with an average of ${avgLength}km`);
